...
let split_aux (sl:string list) : (string * string * string) = match sl with
	| a::b::c::d::e::l -> (a,c, getOption (List.hd (String.split e '_')))
...
let extract_line (line:string) : (string*string*string) = let l = String.split line '\t' in split_aux l;;
...
let rec extract_rec ic  = let line = Stdio.In_channel.input_line ic in match line with
	| Some e -> let triplet = getOption line |> extract_line in triplet::(extract ic)
	| None -> []
;;
